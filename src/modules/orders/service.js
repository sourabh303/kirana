...
function transition(order, toStatus, actorNote) {
  if (!canTransition(order.status, toStatus)) {
    throw new ApiError(409, `Cannot move order from '${order.status}' to '${toStatus}'`);
  }
  order.status = toStatus;
  order.history.push({ status: toStatus, at: new Date().toISOString(), note: actorNote || null });
  return order;
}

// Step 1 & 2: customer selects a shop and places an order from their cart.
function placeOrder(customerId, { deliveryOption, paymentMethod, addressId }) {
  const cartSummary = cartService.summarize(customerId);
  if (!cartSummary.items.length) throw new ApiError(400, 'Cart is empty');
  if (!['home_delivery', 'self_pickup'].includes(deliveryOption)) {
    throw new ApiError(400, 'deliveryOption must be home_delivery or self_pickup');
  }
  ...
  const order = {
    ...
    customerId,
    shopId: cartSummary.shopId,
    items: cartSummary.items.map((i) => ({ ...i, status: 'pending' })),
    pricing: {
      subtotal: cartSummary.subtotal,
      tax: cartSummary.tax,
      platformServiceFee: cartSummary.platformServiceFee,
    ...
    status: STATUS.PLACED,
    history: [{ status: STATUS.PLACED, at: new Date().toISOString(), note: null }],
    createdAt: new Date().toISOString(),
  };
  db.orders.push(order);
  cartService.clear(customerId);

  notificationsService.notify(customerId, 'order_placed', { orderId: order.id });
  return order;
}
...
function listForDeliveryPartner(deliveryPartnerId) {
  return db.orders.filter((o) => o.deliveryPartnerId === deliveryPartnerId);
}

// Step 3: shopkeeper begins reviewing.
function shopkeeperReview(orderId) {
  const order = getById(orderId);
  transition(order, STATUS.UNDER_REVIEW);
  return order;
}

// Step 4: shopkeeper marks specific items unavailable, producing a revised order.
function markUnavailableItems(orderId, unavailableShopProductIds) {
  const order = getById(orderId);
  order.items.forEach((item) => {
    if (unavailableShopProductIds.includes(item.shopProductId)) item.status = 'unavailable';
    else item.status = 'confirmed';
  });
  const anyUnavailable = order.items.some((i) => i.status === 'unavailable');
  transition(order, anyUnavailable ? STATUS.REVISED : STATUS.CONFIRMED);

  // Recompute pricing after removing unavailable items.
  const activeItems = order.items.filter((i) => i.status !== 'unavailable');
  const subtotal = activeItems.reduce((s, i) => s + i.lineTotal, 0);
  order.pricing.subtotal = subtotal;
  order.pricing.total = Number(
    (subtotal + order.pricing.tax + order.pricing.platformServiceFee + order.pricing.deliveryFee).toFixed(2)
  );

  notificationsService.notify(order.customerId, 'order_updated', { orderId: order.id });
  return order;
}

// Step 5 & 6: customer reviews and confirms the (possibly revised) order.
// PRD: "If products are unavailable, customers may add them from another
// nearby store under the same order" — modeled here as linking a follow-up
// order id so the original order still completes independently.
function customerConfirm(orderId, customerId, { addFromAnotherShopOrderId } = {}) {
  const order = getById(orderId);
  if (order.customerId !== customerId) throw new ApiError(403, 'Not your order');
  transition(order, STATUS.CONFIRMED);
  if (addFromAnotherShopOrderId) {
    order.linkedOrderId = addFromAnotherShopOrderId;
  }
  notificationsService.notify(order.customerId, 'order_confirmation_required', { orderId: order.id });
  return order;
}
...
function packOrder(orderId) {
  const order = getById(orderId);
  transition(order, STATUS.PACKED);
  notificationsService.notify(order.customerId, 'order_packed', { orderId: order.id });
  return order;
}

// Step 8: delivery partner assigned (typically by Area Manager / dispatch logic).
function assignDeliveryPartner(orderId, deliveryPartnerId) {
  const order = getById(orderId);
  order.deliveryPartnerId = deliveryPartnerId;
  transition(order, STATUS.DELIVERY_ASSIGNED);
  deliveryService.createAssignment(order.id, deliveryPartnerId);
  notificationsService.notify(order.customerId, 'delivery_assigned', { orderId: order.id });
  notificationsService.notify(deliveryPartnerId, 'delivery_assigned', { orderId: order.id });
  return order;
}

function markOutForDelivery(orderId) {
  const order = getById(orderId);
  transition(order, STATUS.OUT_FOR_DELIVERY);
  notificationsService.notify(order.customerId, 'out_for_delivery', { orderId: order.id });
  return order;
}
...
function markDelivered(orderId) {
  ...
  notificationsService.notify(order.customerId, 'delivered', { orderId: order.id });
  return order;
}

// Step 10: customer confirms receipt, completing the order.
function confirmReceipt(orderId, customerId) {
  const order = getById(orderId);
  if (order.customerId !== customerId) throw new ApiError(403, 'Not your order');
  transition(order, STATUS.COMPLETED);
  return order;
}

function cancel(orderId, note) {
  const order = getById(orderId);
  transition(order, STATUS.CANCELLED, note);
  notificationsService.notify(order.customerId, 'order_cancelled', { orderId: order.id });
  return order;
}
